...
class ESENT_DB:
    ...
    def mountDB(self):
        ...
        self.__DB.seek(0,2)
        self.totalPages = (self.__DB.tell() // self.__pageSize) -2
        ...
        LOG.debug("Total Pages in file: %d" % self.totalPages)
    ...
    def printCatalog(self):
        ...
        print("Number of pages: %d" % self.totalPages)
        print() 
    ...
    def getPages(self, pagecount):
        return_list = []
        for pageNum in range(0,pagecount+2):
            self.__DB.seek((pageNum)*self.__pageSize, 0)
            data = self.__DB.read(self.__pageSize)
            while len(data) < self.__pageSize:
                remaining = self.__pageSize - len(data)
                data += self.__DB.read(remaining)
            ...
            if pageNum <= 0:
                ...
                return_list.append(ESENT_PAGE(self.__DBHeader, data))
        return return_list
    ...
    def getleaf(self, pagedata, tagnum):
        page = pagedata

        #if tagnum >= page.record['FirstAvailablePageTag']:
        #    # No more data in this page, chau
        #    return None
    ...
    def getNextRow(self, cursor):
        ...
        if tag is None:
            ...
            return self.tagToRecord(cursor, tag['EntryData'])

    def tagToRecord(self, cursor, tag):
